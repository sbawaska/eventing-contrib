...
import (
	...
	sourcesv1alpha1 "knative.dev/eventing-contrib/registry/pkg/client/listers/sources/v1alpha1"
...
// controller reconciling v1alpha1.RegistrySource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.RegistrySource. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.RegistrySource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.RegistrySource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.RegistrySource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.RegistrySource) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.RegistrySource resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister sourcesv1alpha1.RegistrySourceLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1alpha1.RegistrySourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.RegistrySources(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.RegistrySource, desired *v1alpha1.RegistrySource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SourcesV1alpha1().RegistrySources(desired.Namespace)
		...
		updater := r.Client.SourcesV1alpha1().RegistrySources(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.RegistrySource) (*v1alpha1.RegistrySource, error) {

	getter := r.Lister.RegistrySources(resource.Namespace)
	...
	patcher := r.Client.SourcesV1alpha1().RegistrySources(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.RegistrySource) (*v1alpha1.RegistrySource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.RegistrySource, reconcileEvent reconciler.Event) (*v1alpha1.RegistrySource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
